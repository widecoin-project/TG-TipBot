        ...
        if update.message.chat.type == "private":
            ...
            if not db.checkUser(user['id']):
                ...
                if address is not None:
                    if checkAdd("wc1q" + address):
                        if ("wc1q" + address) != str(sender_address):
                            if amount is not None:
                                if isFloat(amount):
                                    if float(amount) > float(config.coin['minFee']):
                                        keyboard = [
                                        ...
                                        reply_markup = InlineKeyboardMarkup(keyboard)
                                        ctx.bot.send_message(chat_id=update.message.chat_id,
                                                             text=f"You are about to withdraw {amount} {config.coin['ticker']}, with a fee of {format(float(config.coin['minFee']), '.8f')} {config.coin['ticker']} to {'wc1q' + address}. Please click Yes to confirm",
                                                             reply_markup=reply_markup)
                                    else:
    ...
    if sender == data[3]:
        if data[4] == "t":
            ...
            if data[0] == "Y":
                ...
                sender_balance = 0
                amount = convertToSatoshis(Decimal(data[2])) + fee
                target_address = P2wpkhAddress("wc1q" + data[1])
                ...
                ctx.bot.delete_message(chat_id=chID, message_id=msgID)
                ctx.bot.send_message(chat_id=chID, text=f"You declined withdrawing {data[2]} {config.coin['ticker']} to address {'wc1q' + data[1]}")
